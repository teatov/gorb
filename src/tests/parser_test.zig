...
fn init(allocator: std.mem.Allocator, input: []const u8) !ast.Node {
    const l = try Lexer.init(allocator, input, null);
    var p = Parser.init(allocator, l);
    const program = (try p.parseProgram()).block;
...
test "operator precedence" {
    ...
    for (tests) |expect| {
        ...
        const string = try node.fmt(arena.allocator());
...
test "call expression parameters" {
    ...
    for (tests) |expect| {
        ...
        for (expect.args, 0..) |arg, i| {
            try std.testing.expectEqualStrings(arg, try expr.arguments[i].fmt(arena.allocator()));
        }
    }
}
